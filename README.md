...
- `dots status`: show profile drift, tracked-directory drift, pending changes, destination conflicts, and stale applied state for the active profile. Clean status includes checked-area and tracked-count summaries. Supported secret-scrubbed paths are compared by canonical content, so local-only npm auth token changes do not create drift.
...
- Tracked directory roots may be nested; dirty status output groups paths by the most specific tracked root, with directly tracked files shown under `Individual paths`.
...
- Home-to-repo copies are scanned with pinned Betterleaks default rules. Supported npm auth token findings blank the entire finding line in any file; npmrc `_authToken`, `_auth`, and `_password` lines can also be blanked when stock Betterleaks flags them. Unsupported remaining findings block the copy without printing raw secrets.
...
```sh
scripts/release.sh 0.0.7-alpha.0
git push --atomic origin main v0.0.7-alpha.0
```

The release command builds the GoReleaser archives twice from the Git index,
