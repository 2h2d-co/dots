...
- Added `dots sync` to copy destination-side changes and new files under tracked roots back into the active profile, with `--dry-run` previews and `--force` repo backups for explicit destination-wins conflicts.
